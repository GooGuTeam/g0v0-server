...
from app.database.beatmap.daily_challenge import DailyChallengeStats, DailyChallengeStatsResp
from app.database.beatmap.favourites import FavouriteBeatmapset
from app.database.beatmap.playcounts import BeatmapPlaycounts
from app.database.room.events import Event
from app.database.user.account_history import UserAccountHistory, UserAccountHistoryResp
from app.database.user.counts import CountResp, MonthlyPlaycounts, ReplayWatchedCount
from app.database.user.rank_history import RankHistory, RankHistoryResp, RankTop
from app.database.user.relationship import (
    Relationship as UserRelationship,
    RelationshipResp,
    RelationshipType,
)
from app.database.user.statistics import UserStatistics, UserStatisticsResp
from app.database.user.team import Team, TeamMember
...
if TYPE_CHECKING:
    from app.database.achievement import UserAchievement, UserAchievementResp

if TYPE_CHECKING:
    from app.database.beatmap.favourites import FavouriteBeatmapset
    from app.database.user.relationship import RelationshipResp
...
class User(AsyncAttrs, UserBase, table=True):
    ...
    async def is_user_can_pm(self, from_user: "User", session: AsyncSession) -> tuple[bool, str]:
        from_relationship = (
            await session.exec(
                select(UserRelationship).where(
                    UserRelationship.user_id == from_user.id,
                    UserRelationship.target_id == self.id,
                )
            )
        ).first()
        ...
        relationship = (
            await session.exec(
                select(UserRelationship).where(
                    UserRelationship.user_id == self.id,
                    UserRelationship.target_id == from_user.id,
                )
            )
        ).first()
...
class UserResp(UserBase):
    ...
    async def from_db(
        ...
        *,
        ...
        from app.database.beatmap.favourites import FavouriteBeatmapset
        from app.database.score import Score, get_user_first_score_count
        ...
        u.follower_count = (
            await session.exec(
                ...
                .where(
                    UserRelationship.target_id == obj.id,
        ...
        if "session_verified" in include:
            from app.service.session.verification_service import LoginSessionService
