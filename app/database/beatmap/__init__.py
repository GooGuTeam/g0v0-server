...
from .playcounts import BeatmapPlaycounts
...
class Beatmap(BeatmapBase, table=True):
    ...
    beatmapset: Beatmapset = Relationship(
        sa_relationship=relationship("Beatmapset", back_populates="beatmaps", lazy="joined")
    )
    failtimes: FailTime | None = Relationship(
        sa_relationship=relationship("FailTime", back_populates="beatmap", lazy="joined")
    )
