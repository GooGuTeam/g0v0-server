...
from ipaddress import (
    IPv4Address,
    IPv6Address,
    ip_address as parse_ip_address,
)
import secrets
import string
...
from app.database.auth.verification import EmailVerification, LoginSession
from app.log import logger
from app.service.email.email_queue import email_queue  # 导入邮件队列
...
class LoginSessionService:
    ...
    @classmethod
    def _trusted_device_key(cls, user_id: int, fingerprint: str) -> str:
        return f"login_trust:{user_id}:{fingerprint}"

    @staticmethod
    def make_device_fingerprint(ip_address: str | None, user_agent: str | None) -> str | None:
        """生成设备指纹，用于记住已验证的设备"""
        ...
        if isinstance(parsed_ip, IPv4Address):
            prefix = ".".join(str(parsed_ip).split(".")[:3])
        elif isinstance(parsed_ip, IPv6Address):
            prefix = ":".join(parsed_ip.exploded.split(":")[:4])
        else:
            prefix = ip_address.split(":")[0] if ":" in ip_address else ".".join(ip_address.split(".")[:3])

        raw = f"{prefix}|{user_agent}"
        return hashlib.sha256(raw.encode("utf-8")).hexdigest()

    @classmethod
    async def is_trusted_device(cls, redis: Redis, user_id: int, fingerprint: str | None) -> bool:
        if not fingerprint:
            return False
        key = cls._trusted_device_key(user_id, fingerprint)
        return bool(await redis.exists(key))

    @classmethod
    async def remember_trusted_device(cls, redis: Redis, user_id: int, fingerprint: str | None) -> None:
        if not fingerprint:
            return

        ttl_hours = settings.login_trust_duration_hours
        if ttl_hours <= 0:
            return
